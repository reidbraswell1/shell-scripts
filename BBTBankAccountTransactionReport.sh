...
cleanUpTextFile()
{
  ...
  awk -F"${delimiterChar}" -v delimiter="${delimiterChar}" '{
                ...
                #Remove all "PAY[A-Z]" from the description field
                 gsub("PAY[A-Z]*","",$4)

                if (substr($5, 1, 1) == "(")
                {
                   #Remove leading "(" from amount field
                    gsub("^\\(","",$5)

                   #Remove trailing ")" from amount field
                    gsub("\\)$","",$5)
                ...
                date = month "/" day "/" year
                $1 = date
                print $1 delimiter $2 delimiter $3 delimiter $4 delimiter $5 delimiter $6
...
sortTextFile()
{
   ...
   #Field 4 is Desc, Field 1 is Date
   sort -t "${delimiterChar}" -k4,4 -k1,1r -o "${sortedOutputFile}" "${inputFile}"
}
